# Question 0
## Creating namespaces in both k8s and dk8s cluster
1. sun
2. earth
3. moon
4. pluto
...
## Create a pod named liveness-exec, image k8s.gcr.io/busybox, the box will first create a file "tmp/healthy", then remove it after 30 then sleep 600, check if it is alive every 5 seconds and make sure it start 5 seconds after the pod is created. using touch and cat command. run command and see if it is successful.  
...
## Create a busybox pod checks the HTTP and TCPSOcket for liveness
...
startup probe example. main thing is the failure threahold. The trick is to set up a startup probe with the same command, HTTP or TCP check, with a ilureThfareshold * periodSeconds long enough to cover the worse case startup time.Protect slow starting containers with startup probes
...
## Create a new Pod named `hello` with the image `bonomat/nodejs-hello-world` that exposes the port 3000. Provide the name `nodejs-port` for the container port. Add a Readiness Probe that checks the URL path / on the port referenced with the name `nodejs-port` after a 2 seconds delay. You do not have to define the period interval, if the port cannot start for 30 seconds, fail it.Add a Liveness Probe that verifies that the app is up and running every 8 seconds by checking the URL path / on the port referenced with the name `nodejs-port`. The probe should start with a 5 seconds delay. Shell into container and curl `localhost:3000`. Write down the output. Exit the container. Retrieve the logs from the container. Write down the output.Implement a liveness-probe which checks the container to be reachable on port 80. Initially the probe should wait 10, periodically 15 seconds. 
